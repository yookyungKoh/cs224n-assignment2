...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        while len(minibatch_parses) > 0:
            transitions = model.predict(minibatch_parses)
            for i, p in enumerate(transitions):
                minibatch_parses[i].parse_step(p)
            minibatch_parses = [parse for parse in minibatch_parses if len(parse.stack)>1 or len(parse.buffer)>0]
